...
//    for (int i = 0; i <= settings.rs; i++) {
        ...
        LinkedList<Edge>[] partitionSortedEdges = HovercutPartitioner.partitionWithWindow(
                state,
                splits,
                heuristic,
                settings.window,
        ...
        writeToFile(settings.output, partitionSortedEdges);
//    }
        return state;
    }

    public static void writeToFile(
            String output, LinkedList<Edge>[] partitions) throws FileNotFoundException {
        String file = output + "-ep.txt";
        File f = new File(file);

        try (PrintWriter writer1 = new PrintWriter(new FileOutputStream(f, false))) {
            for (int i = 0; i < partitions.length; i++) {
                for (Edge e : partitions[i]) {
                    writer1.append(String.format("%d\t%d\t%d", e.getSrc(), e.getDst(), i));
                    writer1.append("\n");
                }
            }
            writer1.flush();
